...
export class ImageUploadMiddleware implements NestMiddleware {
  ...
  private readonly logger = new Logger(ImageUploadMiddleware.name);

  constructor() {
    this.logger.log('Initializing ImageUploadMiddleware');
    ...
    this.logger.log('ImageUploadMiddleware initialized successfully');
  }

  use(req: Request, res: Response, next: NextFunction) {
    this.logger.log(`Request received: ${req.method} ${req.path}`);
    
    // Only use this middleware for image upload routes
    if (req.path.includes('/images/upload')) {
      this.logger.log('Image upload route detected, applying file upload middleware');
