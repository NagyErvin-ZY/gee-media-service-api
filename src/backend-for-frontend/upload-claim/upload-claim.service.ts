...
export class UploadClaimService {
  ...
  async createClaim(createClaimDto: CreateClaimDto): Promise<ClaimResponseDto> {
    ...
    // Use formatClaimResponse to maintain consistent response format
    return this.formatClaimResponse(newClaim);
  }
  ...
  /**
   * Creates a facade for an image asset with ID transformation
   * @param asset The original image asset
   * @returns The transformed asset with id instead of _id
   */
  private createImageAssetFacade(asset: any): any {
    if (!asset) return null;
    
    // Create a plain object copy
    const facade = JSON.parse(JSON.stringify(asset));
    
    // Replace _id with id with prefix
    if (facade._id) {
      facade.id = `image-${facade._id}`;
      delete facade._id;
    }
  ...
  /**
   * Creates a facade for a video asset with ID transformation
   * @param asset The original video asset
   * @returns The transformed asset with id instead of _id
   */
  private createVideoAssetFacade(asset: any): any {
    if (!asset) return null;
   ...
   */
  async formatClaimResponse(claim: UploadClaim): Promise<ClaimResponseDto> {
    ...
    // Look up associated image asset if one exists
    const imageAsset = await this.imageAssetModel.findOne({ claimId: claim.claimId }).exec();
    if (imageAsset) {
      response.image = this.createImageAssetFacade(imageAsset.toObject());
    }

    // Look up associated video asset if one exists
    const videoAsset = await this.videoAssetModel.findOne({ claimId: claim.claimId }).exec();
    if (videoAsset) {
      response.video = this.createVideoAssetFacade(videoAsset.toObject());
    }
